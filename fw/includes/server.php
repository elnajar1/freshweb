...
if(isset($_POST['signup'])){
    ...
    $stmt = $pdo->prepare('SELECT email  FROM fw_users WHERE email = ?');
    $stmt->execute([$email]);
   ...
   if (empty($password_1)){
    ...
    //if there are no errors,save user to db
      if (count($myerrors)==0){
       ...
       if(!is_dir('uploads')){
        ...
        $sql = "INSERT INTO fw_users (name,  username, avatar,email, password) VALUES( :yourname, :username,:avatar, :email, :password)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(['yourname' => $yourname , 'username' => $username ,'avatar' =>$avatar, 'email' => $email , 'password' => $password]);
    ...
    if (empty($password)){
     ...
     if (count($loginErrors) == 0) {
   	 ...
   	 $sql ="SELECT * FROM fw_users WHERE email = :email";
    	$stmt = $pdo->prepare($sql);
...
if(isset($_GET['fb_login'])  && isset($fb_user_data['id'])   ){
	...
	//check if it in DB
	$sql ="SELECT id FROM fw_users WHERE fb_id = ? ";
	$stmt = $pdo->prepare($sql);
	...
	//frist time
	if( $count == 0 ){
		$sql = "INSERT INTO fw_users ( fb_id, fb_name,  fb_first_name, fb_last_name, fb_avatar, fb_email, fb_access_token ) VALUES(?,?,?,?,?,?,? )";
		$stmt = $pdo->prepare($sql);
...
if(isset( $user_id )){
    ...
    if(isset($user['avatar'])){
    	...
    	$user_avatar = $root .  "/layout/img/user.jpeg" ;
    }
